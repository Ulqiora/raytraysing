...
//	glEnable(GL_DEPTH_TEST);
    ...
    try {
        cube = new s21::Model(QFileDialog::getOpenFileName().toStdString());
//        cube = new s21::Model("");
...
{
    if (!program.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shaders/vertexShader.glsl")) {
        qDebug() << __FILE__<<__FUNCTION__<< " add vertex shader file failed.";
        close();
    }
    if (!program.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shaders/fragmentShader.glsl")) {
